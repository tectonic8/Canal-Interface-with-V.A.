var VA = (function() {
  userSwitchData = [[0.01073028, 0.01550475, 0.01930014, 0.02298981, 0.02690030, 0.03131760, 0.03664462, 0.04365011, 0.05433514, 0.07851171],
...
return {
  VirtualAgent: class {
    constructor(x, y, d, behavior, st) {
      ...
      this.position = {x:Map.d2x(d), y:Map.d2y(d)};
    ...
    /* Choose the current target for the virtual agent. */
      ...
      for (var i = 0; i <= 19; i++) {
        if ((Map.targetTracker[i] == 0) || (Map.targetTracker[i] == 2)) thereAreNoAvailableTargets = false;
      }
      ...
      if (Map.targetTracker[this.target.index] == 0) this.task = 0;
      else if (Map.targetTracker[this.target.index] == 2) this.task = 1;
    ...
    closestVerify() {
      ...
      for (var i = 0; i <= 19; i++) {
        ...
        if (Map.targetTracker[i] == 2) {
          ...
          if (nextDistance < leastDistance) {
            leastDistance = nextDistance;
            verifyTarget = [0, 0, i, Map.targetDCoordinates[i]];
          }
        }
      }
    ...
    slowDown(initializeAfter, oldD) {
      if (this.magnitude < 0.005 && this.magnitude > -0.005) {
        ...
        var x = Map.d2x(this.mapProportion);// + 14;
        var y = Map.d2y(this.mapProportion);// - 4412;
        var dx = x - Map.d2x(oldD);
        var dy = y - Map.d2y(oldD);
        this.position.x = x;
        this.position.y = y;
    ...
    updateMap(msg) {
      this.sprite.style.left = this.position.x - Map.gpRecorder[2] + "px";
      this.sprite.style.top = this.position.y - Map.gpRecorder[3] + "px";
      Map.miniSprite2coords[0] += (msg.x*0.047);
      Map.miniSprite2coords[1] += (msg.y*0.045);
    ...
    tagAndVerify() { //This handle the targets for the va
      if (this.fastForward) {
        if (this.onTarget) {
          ...
          if (this.task === 0 && this.framesElapsed >= framesRequiredTag) {
            if (Map.tagTargets[this.target.index]) {
              Map.tagTargets[this.target.index].remove();
            }
            if (Map.targetSprites[this.target.index]) {
              Map.targetSprites[this.target.index].remove();
            }
            Map.targetTracker[this.target.index] = 1;
            Map.userVerifyTargets [this.target.index] = document.createElement('IMG');
            Map.userVerifyTargets [this.target.index].setAttribute('id', 'userVerifyTarget' + this.target.index);
            divWrapper.appendChild(Map.userVerifyTargets[this.target.index]);
            Map.targetSprites[this.target.index] = document.createElement('IMG');
            Map.targetSprites[this.target.index].setAttribute('id', 'miniTarget' + this.target.index);
            divWrapper.appendChild(Map.targetSprites[this.target.index]);
            new Target(Map.targetDCoordinates[this.target.index], 'userVerifyTarget', this.target.index, Map.targetTracker[this.target.index]);
            this.onTarget = false;
            ...
            Map.vaVerifyTargets[this.target.index].remove();
            Map.targetSprites[this.target.index].remove();
            Map.targetTracker[this.target.index] = 3;
            Map.targetsVerified += Map.tagTracker[this.target.index];
      ...
      if (!this.onTarget) {
        ...
        if ((this.task == 0)  &&  ((endTime2 - this.tagAndVerifyDelayStart) >= (timeToTag))) {
          if (Map.tagTargets[this.target.index]) { Map.tagTargets[this.target.index].remove(); }
          if (Map.targetSprites[this.target.index]) { Map.targetSprites[this.target.index].remove(); }
          Map.targetTracker[this.target.index] = 1; //this becomes a user verifies target.
